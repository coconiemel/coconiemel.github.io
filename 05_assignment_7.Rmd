...
For a good analysis of the different, but related data, they need to be joined together in one table with the important columns. I created two joined tables, flu with dengue and flu with gapminder. The flu and dengue table shows only four countries, this is because only those four countries occurred in both datasets. This is also the case for the gapminder and flu table, but here are more countries that occurred in both.
...
The following graphs give an overview of amount of cases for flu or dengue. Figure 3 and 5 shows the amount of flu cases per continent and figure 4 and 6 show the average cases per year per country. 
Mind that the counted cases aren't shown in proportion of the population.
...
Figure 3 shows that the most cases are in the Americas (North and South), which is made clear in figure 4 that shows Mexico with a high amount of cases.
...
gapminder_dengue %>%
  ggplot(aes(x=continent,y=dengue_cases, group=continent, fill=continent)) + 
  geom_col(stat="identity")+ 
  labs(title = "Dengue cases per continent", 
       x="Continent", 
       y="Average dengue cases per continent",
...
```
Figure 5 shows a small difference between Asia and the Americas and figure 6 shows a big spike around 2009 for all countries shown.
